...
exports.addVitalSigns = async (req, res) => {
    try {
        const { patientId, nurseId, temperature, heartRate, bloodPressure, respiratoryRate } = req.body;
        ...
        if (isNaN(parsedTemperature) || isNaN(parsedHeartRate) || isNaN(systolicBP) || isNaN(diastolicBP) || isNaN(parsedRespiratoryRate)) {
            return res.status(400).json({ error: 'Invalid input. Please ensure all values are numbers.' });
        }
        ...
        const vitalSignsInput = tf.tensor2d([[parsedTemperature, parsedHeartRate, systolicBP, parsedRespiratoryRate]], [1, 4]);
        ...
        if (systolicBP >= 140 || diastolicBP >= 90) conditions.add('Hypertension - Requires Review');
        if (parsedRespiratoryRate > 20) conditions.add('Tachypnea - Requires Review');
...
exports.getVitalSignsByPatient = async (req, res) => {
    try {
        ...
        const vitalSigns = await VitalSigns.find({ patientId }).populate('nurseId', 'name').sort({ visitDate: -1 });
